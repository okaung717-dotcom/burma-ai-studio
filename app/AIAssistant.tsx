        ...
        <div className="mb-4 flex h-[600px] max-h-[calc(100vh-7rem)] w-[calc(100vw-2.5rem)] max-w-[420px] flex-col overflow-hidden rounded-[2rem] border border-[#be9537]/30 bg-white shadow-[0_24px_80px_rgba(145,25,35,0.22)] dark:border-[#be9537]/20 dark:bg-[#100708]">
          <div className="flex shrink-0 items-center justify-between bg-[#911923] px-5 py-4 text-white">
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <p className="truncate text-xs text-white/75">English / မြန်မာ • Instant answers</p>
              </div>
            </div>
          ...
          <div className="shrink-0 border-b border-[#be9537]/20 bg-[#fff9f0] px-5 py-3 dark:bg-[#1a0b0e]">
            <div className="flex flex-wrap gap-2">
              {quick[activeLang].map((prompt) => (
                <button key={prompt} onClick={() => sendMessage(prompt)} className="rounded-full border border-[#be9537]/30 bg-white px-3 py-1.5 text-xs font-bold text-[#911923] transition-colors hover:bg-[#fff3e3] dark:bg-[#241113] dark:text-[#e3bc61] dark:hover:bg-[#331719]">
                  {prompt}
                </button>
              ))}
            </div>
          ...
          <div className="shrink-0 border-t border-[#be9537]/20 bg-white p-4 shadow-[0_-18px_40px_rgba(145,25,35,0.08)] dark:bg-[#1a0b0e]">
            <form onSubmit={handleSubmit} className="flex items-center gap-2">
              <input value={input} onChange={(event) => setInput(event.target.value)} placeholder={activeLang === "MM" ? "မေးချင်တာရေးပါ..." : "Ask anything about the website..."} className="min-w-0 flex-1 rounded-2xl border border-[#be9537]/30 bg-[#fff9f0] px-4 py-3 text-sm text-gray-900 outline-none transition-shadow focus:ring-2 focus:ring-[#911923]/30 dark:bg-[#100708] dark:text-white" />
              <button type="submit" className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-[#911923] text-white transition-transform hover:scale-105" aria-label="Send message"><Send className="h-5 w-5" /></button>
            </form>
          </div>
        </div>
      )}
