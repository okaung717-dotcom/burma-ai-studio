...
function mm(text: string) { return /[\u1000-\u109F]/.test(text); }
function has(text: string, words: string[]) { return words.some((word) => text.includes(word)); }
function send(reply: string, status = 200) { return Response.json({ reply }, { status }); }
...
function related(text: string) {
  ...
  return has(q, ["ai", "video", "ad", "ads", "commercial", "promo", "brand", "service", "price", "cost", "quote", "portfolio", "sample", "contact", "script", "concept", "tiktok", "reels", "shorts", "product", "hotel", "restaurant", "bar", "cafe", "music", "song", "shop", "business", "cinematic", "presenter", "delivery", "revision", "duration", "deadline", "burma", "studio", "website", "ဗီဒီယို", "ကြော်ငြာ", "ဈေး", "စျေး", "နမူနာ", "လက်ရာ", "ဆက်သွယ်", "ဇာတ်ညွှန်း", "ဟိုတယ်", "စားသောက်", "ဆိုင်", "ဘား", "ကဖေး", "သီချင်း", "လုပ်ငန်း", "လုပ်ချင်", "ဖန်တီး", "ပို့", "ရက်", "ပြင်ဆင်", "ပရောဂျက်"]);
}
...
function answer(text: string) {
  ...
  if (has(q, ["hotel", "room", "booking", "resort", "lobby", "pool", "ဟိုတယ်", "အခန်း", "ဧည့်", "အပန်းဖြေ"])) {
    return isMM
  ...
  if (has(q, ["price", "cost", "how much", "budget", "quote", "ဈေး", "စျေး", "ဘယ်လောက်", "ကုန်ကျ"])) {
    return isMM
  ...
  if (has(q, ["contact", "phone", "telegram", "viber", "email", "ဆက်သွယ်", "ဖုန်း", "တယ်လီဂရမ်"])) {
    return isMM
      ? `တိုက်ရိုက်ဆက်သွယ်နိုင်ပါတယ် — ${CONTACT}။\n\nProject idea ပို့ပေးရင် video direction နဲ့ quote ကိုပြန်ညှိပေးပါမယ်။`
      : `You can contact Burma AI Studio directly — ${CONTACT}.\n\nSend your project idea and we’ll suggest the video direction and quote.`;
  }

  if (has(q, ["music", "song", "artist", "သီချင်း", "အဆိုတော်"])) {
    return isMM
  ...
  if (has(q, ["bar", "restaurant", "cafe", "food", "drink", "စားသောက်", "ဆိုင်", "ဘား", "ကဖေး"])) {
    return isMM
  ...
  if (has(q, ["script", "idea", "concept", "caption", "စာသား", "ဇာတ်ညွှန်း", "အကြံ", "စိတ်ကူး"])) {
    return isMM
  ...
  if (has(q, ["portfolio", "sample", "example", "နမူနာ", "လက်ရာ", "ပြခန်း"])) {
    return isMM
      ? "နမူနာတွေကို Portfolio page မှာကြည့်နိုင်ပါတယ်။ ကြိုက်တဲ့ style ကို reference အဖြစ်ပို့ပေးရင် သင့် project နဲ့ကိုက်အောင် concept ညှိပေးနိုင်ပါတယ်။\n\nAI presenter style ကြိုက်လား၊ cinematic style ကြိုက်လား?"
      : "You can check examples on the Portfolio page. If you send a style you like, we can adapt the concept for your project.\n\nDo you prefer AI presenter style or cinematic style?";
  }
...
export async function POST(request: Request) {
  ...
  if (!question) return send("Please send a message first.", 400);
  return send(answer(question));
}
