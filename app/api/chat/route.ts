...
Every answer must end with a complete sentence. Never stop mid-sentence or mid-word.
...
function getLastUserQuestion(messages: ChatMessage[]) { return [...messages].reverse().find((message) => message.role === "user")?.content || ""; }
function hasAny(text: string, words: string[]) { return words.some((word) => text.includes(word)); }
function compact(text: string) { return text.replace(/\n{3,}/g, "\n\n").trim(); }
...
function fallback(question: string) {
  ...
  if (hasAny(q, ["price", "cost", "how much", "budget", "ဈေး", "စျေး", "ဘယ်လောက်", "ကုန်ကျ", "ပက်ကေ့", "package"])) {
    return mm
      ...
      : "Price depends on duration, scene count, voice/dialogue, presenter/character, realism level, deadline and revisions. It is better to quote based on the exact project.\n\nSend your product/service, platform, duration and reference style. I will suggest the best option for your budget.";
  }
  ...
  if (hasAny(q, ["bar", "beer", "cocktail", "restaurant", "cafe", "စားသောက်", "ဘား", "ကဖေး", "ဆိုင်", "အရက်", "drink", "food"])) {
    return mm
      ...
      : "For a bar or restaurant ad, a 15-30s Reels/TikTok video with night ambience, drink or food close-ups, customer mood and a premium logo ending will work best.\n\nSend the shop name, location, product focus and preferred vibe.";
  }
  ...
  if (hasAny(q, ["portfolio", "sample", "example", "နမူနာ", "လက်ရာ", "ပြခန်း"])) {
    return mm
      ...
      : "You can check the Portfolio page for examples. If you like a style, send it as a reference and we can adapt the concept for your product or service.\n\nDo you prefer AI presenter style or cinematic style?";
  }
  ...
  try {
    const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent`, {
      ...
      body: JSON.stringify({
        ...
        contents,
        generationConfig: { temperature: 0.68, topP: 0.9, maxOutputTokens: 650 },
      }),
    });
