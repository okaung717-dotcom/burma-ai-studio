    ...
    <main className="min-h-screen bg-[#fff9f0] px-5 py-10 text-[#1a0b0e] dark:bg-[#100708] dark:text-[#fff7eb]">
      <div className="mx-auto max-w-6xl space-y-6">
        <section className="rounded-[2rem] border border-[#be9537]/25 bg-white p-6 shadow-xl dark:bg-[#1a0b0e]">
          ...
          <p className="mt-3 text-gray-600 dark:text-[#d8c4a3]">View messages, manage portfolio YouTube videos, reply by email, refresh inbox, and export data.</p>
        </section>
        ...
        <section className="rounded-[2rem] border border-[#be9537]/25 bg-white p-5 shadow-xl dark:bg-[#1a0b0e]">
          <h2 className="mb-4 text-2xl font-black">Login / Inbox</h2>
        ...
        <section className="rounded-[2rem] border border-[#be9537]/25 bg-white p-5 shadow-xl dark:bg-[#1a0b0e]">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div><h2 className="text-2xl font-black">Portfolio Video Manager</h2><p className="mt-1 text-sm text-gray-500 dark:text-[#d8c4a3]">Paste YouTube video ID or URL, edit titles, then save.</p></div>
            <div className="flex flex-wrap gap-2"><button onClick={() => void loadPortfolio()} className="rounded-xl border border-[#be9537]/30 px-4 py-2 font-bold text-[#911923] dark:text-[#e3bc61]">Load Portfolio</button><button onClick={() => setPortfolio((current) => [...current, emptyItem()])} className="rounded-xl border border-[#be9537]/30 px-4 py-2 font-bold text-[#911923] dark:text-[#e3bc61]">Add Video</button><button disabled={savingPortfolio || portfolio.length === 0} onClick={() => void savePortfolio()} className="rounded-xl bg-[#911923] px-4 py-2 font-bold text-white disabled:opacity-50">Save Portfolio</button></div>
          </div>
          ...
          <div className="mt-5 space-y-4">
            {portfolio.map((item, index) => (
              <div key={item.id || index} className="rounded-2xl border border-[#be9537]/20 bg-[#fffdf8] p-4 dark:bg-[#100708]">
                <div className="grid gap-3 md:grid-cols-2">
                  <input value={item.src} onChange={(event) => updatePortfolio(index, "src", event.target.value)} placeholder="YouTube ID or URL" className="rounded-xl border border-[#be9537]/30 bg-white px-4 py-3 outline-none dark:bg-[#1a0b0e]" />
                  <input value={item.titleEN} onChange={(event) => updatePortfolio(index, "titleEN", event.target.value)} placeholder="English title" className="rounded-xl border border-[#be9537]/30 bg-white px-4 py-3 outline-none dark:bg-[#1a0b0e]" />
                  <input value={item.descEN} onChange={(event) => updatePortfolio(index, "descEN", event.target.value)} placeholder="English description" className="rounded-xl border border-[#be9537]/30 bg-white px-4 py-3 outline-none dark:bg-[#1a0b0e]" />
                  <input value={item.titleMM} onChange={(event) => updatePortfolio(index, "titleMM", event.target.value)} placeholder="Myanmar title" className="rounded-xl border border-[#be9537]/30 bg-white px-4 py-3 outline-none dark:bg-[#1a0b0e]" />
                  <input value={item.descMM} onChange={(event) => updatePortfolio(index, "descMM", event.target.value)} placeholder="Myanmar description" className="rounded-xl border border-[#be9537]/30 bg-white px-4 py-3 outline-none dark:bg-[#1a0b0e] md:col-span-2" />
                </div>
                <div className="mt-3 flex flex-wrap items-center justify-between gap-3"><label className="flex items-center gap-2 text-sm font-bold"><input type="checkbox" checked={Boolean(item.featured)} onChange={(event) => updatePortfolio(index, "featured", event.target.checked)} /> Featured</label><button onClick={() => setPortfolio((current) => current.filter((_, itemIndex) => itemIndex !== index))} className="rounded-xl border border-red-200 px-4 py-2 text-sm font-bold text-red-700">Remove</button></div>
              </div>
            ))}
          </div>
        </section>
