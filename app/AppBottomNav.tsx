    ...
    <div className="fixed inset-x-0 bottom-0 z-[9998] px-3 pb-[calc(env(safe-area-inset-bottom,0px)+0.65rem)] md:hidden">
      <nav className="mx-auto flex max-w-[430px] items-center justify-between rounded-[2rem] border border-[#be9537]/30 bg-[#fffdf8]/92 px-2 py-2 shadow-[0_18px_55px_rgba(26,11,14,0.18)] backdrop-blur-2xl dark:border-[#e3bc61]/25 dark:bg-[#100708]/92">
        {items.map(({ href, label, Icon }) => {
          ...
          return (
            <Link key={href} href={href} className={`flex min-w-[4.1rem] flex-col items-center justify-center rounded-3xl px-3 py-2 text-[10px] font-extrabold transition-all ${active ? "bg-[#911923] text-white shadow-lg shadow-[#911923]/25 dark:bg-[#e3bc61] dark:text-[#100708]" : "text-[#79695d] hover:bg-[#fff3e3] hover:text-[#911923] dark:text-[#d8c4a3] dark:hover:bg-white/8"}`}>
              <Icon className="mb-1 h-[18px] w-[18px]" />
              <span className="leading-none">{label}</span>
            </Link>
          );
        })}
        <Link href="/contact" className="flex min-w-[4.1rem] flex-col items-center justify-center rounded-3xl bg-[#be9537] px-3 py-2 text-[10px] font-extrabold text-[#100708] shadow-lg shadow-[#be9537]/25">
